...
> `genProgress` 是当前进度文案（前端每 2.5 秒轮询一次），失败时 `genError` 写明原因。
...
一次生成大约二十秒到一分钟，会真实调用模型与高德接口十余次。开发期想看完整产出，在项目根目录执行
`npm run try:generate -- <用户名> <天数>`，终端会打印每一步工具调用与最终行程。
...
```bash
# 手动跑一次真实生成，终端会打印每一次工具调用与最终行程
npm run try:generate -- <用户名> <天数>
...
> 模型偶尔会输出无法直接解析的 JSON（前后夹解释、字符串里有裸换行、或写到一半撞上长度上限被截断）。
> 编排层对此有三重防线：解析器按「原样 → 修小毛病 → 补全截断」逐级尝试；
> 仍失败则**带着失败原因请模型重新输出一次**（不重跑工具查询）；
> 两次都失败才报错，并把模型原始输出存档到 `apps/server/.debug/` 供事后排查。
> 同时请求里显式指定了 `max_tokens`，避免落到厂商默认值（DeepSeek 默认 4096）导致长行程被截断。
